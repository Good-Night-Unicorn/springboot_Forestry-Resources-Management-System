		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"20px","margin":"0px 0 0","overflow":"hidden","borderRadius":"0px","background":"none","width":"auto","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px","borderColor":"#355749","color":"#c2c2c2","textAlign":"center","flexDirection":"row","display":"flex","borderRadius":"4px","flexWrap":"wrap","background":"#fff","borderWidth":"2px 0 0","width":"100%","fontSize":"inherit","borderStyle":"solid"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#797979","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">评论内容</label>
				...
				<el-row :style='{"padding":"20px","borderColor":"#355749","color":"#c2c2c2","textAlign":"center","flexDirection":"row","display":"flex","borderRadius":"4px","flexWrap":"wrap","background":"#fff","borderWidth":"2px 0 0","width":"100%","fontSize":"inherit","borderStyle":"solid"}'>
					<el-button :style='{"cursor":"pointer","padding":"0 20px","borderColor":"#1576c2","margin":"0 10px","color":"#fff","minWidth":"80px","transition":"all 0.3s","outline":"none","borderRadius":"4px","background":"#78CD51","borderWidth":"0 0 0px","width":"auto","fontSize":"14px","borderStyle":"solid","height":"34px"}' type="success" @click="backClick()">返回</el-button>
				</el-row>
				...
				<el-row class="actions" :style='{"padding":"20px 20px 10px","margin":"20px 0 0","borderColor":"#355749","color":"#fff","textAlign":"left","flexDirection":"row","display":"block","borderRadius":"4px","flexWrap":"wrap","background":"#fff","borderWidth":"2px 0 0","width":"100%","fontSize":"inherit","borderStyle":"solid"}'>
					<el-button class="add" v-if="isAuth('discusslinyezhengce','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('discusslinyezhengce','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #e1e8ee","padding":"0px","margin":"0px auto","borderColor":"#355749","borderRadius":"4px","background":"rgba(255,255,255,1)","borderWidth":"2px 0 0","width":"calc(100% - 40px)","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discusslinyezhengce','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('discusslinyezhengce','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
...
import AddOrUpdate from "./add-or-update";
	...
	this.$router.push({path:'/discussdiscusslinyezhengce',query:{refid:id}});
    },
	backClick(){
		history.back()
	},
	discusslinyezhengcestatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'discusslinyezhengce/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
    ...
    async deleteHandler(id ,refid ) {
		...
		await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
			...
			await this.$http({
				url: "discusslinyezhengce/delete",
				method: "post",
				data: ids
			}).then(async ({ data }) => {
				if (data && data.code === 0) {
					this.$http({
						url: 'linyezhengce/info/' + refid,
						method: "get",
					}).then(res=>{
