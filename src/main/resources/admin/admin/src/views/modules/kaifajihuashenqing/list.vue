<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"0px 0px 20px","overflow":"hidden","color":"#666","background":"#e5e6e6","fontSize":"14px","position":"relative"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"20px","margin":"0px 0 0","overflow":"hidden","borderRadius":"0px","background":"none","width":"auto","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px","borderColor":"#355749","color":"#c2c2c2","textAlign":"center","flexDirection":"row","display":"flex","borderRadius":"4px","flexWrap":"wrap","background":"#fff","borderWidth":"2px 0 0","width":"100%","fontSize":"inherit","borderStyle":"solid"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' class="select" label="开发进度" prop="kaifajindu">
						<label :style='{"margin":"0 10px 0 0","color":"#797979","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">开发进度</label>
						<el-select clearable v-model="searchForm.kaifajindu" placeholder="请选择开发进度" >
							<el-option v-for="(item,index) in kaifajinduOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"20px 20px 10px","margin":"20px 0 0","borderColor":"#355749","color":"#fff","textAlign":"left","flexDirection":"row","display":"block","borderRadius":"4px","flexWrap":"wrap","background":"#fff","borderWidth":"2px 0 0","width":"100%","fontSize":"inherit","borderStyle":"solid"}'>
					<el-button class="add" v-if="isAuth('kaifajihuashenqing','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('kaifajihuashenqing','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('kaifajihuashenqing','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						审核
					</el-button>

					<el-button class="btn18" v-if="isAuth('kaifajihuashenqing','开发进度')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"0px solid #e1e8ee","padding":"0px","margin":"0px auto","borderColor":"#355749","borderRadius":"4px","background":"rgba(255,255,255,1)","borderWidth":"2px 0 0","width":"calc(100% - 40px)","borderStyle":"solid"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('kaifajihuashenqing','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="shenqingcailiao" label="申请材料">
						<template slot-scope="scope">
							<el-button v-if="scope.row.shenqingcailiao" type="text" size="small" @click="download(scope.row.shenqingcailiao)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kaifajihuashenqing','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('kaifajihuashenqing','开发报告')" @click="kaifaliyongbaogaoCrossAddOrUpdateHandler(scope.row,'cross','是','','kaifajindu','已开发','已开发,未开发'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								开发报告
							</el-button>
							<el-button class="edit" v-if=" isAuth('kaifajihuashenqing','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('kaifajihuashenqing','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<kaifaliyongbaogao-cross-add-or-update v-if="kaifaliyongbaogaoCrossAddOrUpdateFlag" :parent="this" ref="kaifaliyongbaogaoCrossaddOrUpdate"></kaifaliyongbaogao-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="kaifajinduChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import kaifaliyongbaogaoCrossAddOrUpdate from "../kaifaliyongbaogao/add-or-update";
	export default {
		data() {
			return {
				kaifajinduOptions: [],
				...
				kaifaliyongbaogaoCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			kaifaliyongbaogaoCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			kaifaliyongbaogaoCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.kaifaliyongbaogaoCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','kaifajihuashenqing');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kaifaliyongbaogaoCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kaifaliyongbaogaoCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var kaifajinduChart1 = echarts.init(document.getElementById("kaifajinduChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                kaifajinduChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    kaifajinduChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
          ...
          if(this.searchForm.kaifajindu!='' && this.searchForm.kaifajindu!=undefined){
            params['kaifajindu'] = this.searchForm.kaifajindu
          }
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "kaifajihuashenqing/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
	...
	kaifajihuashenqingstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'kaifajihuashenqing/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
