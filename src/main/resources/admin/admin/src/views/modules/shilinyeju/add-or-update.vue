...
export default {
	data() {
		...
		return {
			...
			rules: {
				linyejumingcheng: [
					{ required: true, message: '林业局名称不能为空', trigger: 'blur' },
				],
				mima: [
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `shilinyeju/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shilinyejuCrossAddOrUpdateFlag = false;
											this.parent.search();
