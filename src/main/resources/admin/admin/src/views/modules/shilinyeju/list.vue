			...
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"20px","margin":"0px 0 0","overflow":"hidden","borderRadius":"0px","background":"none","width":"auto","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px","borderColor":"#355749","color":"#c2c2c2","textAlign":"center","flexDirection":"row","display":"flex","borderRadius":"4px","flexWrap":"wrap","background":"#fff","borderWidth":"2px 0 0","width":"100%","fontSize":"inherit","borderStyle":"solid"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#797979","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">林业局名称</label>
						<el-input v-model="searchForm.linyejumingcheng" placeholder="林业局名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#797979","display":"inline-block","lineHeight":"32px","fontSize":"inherit","fontWeight":"500","height":"32px"}' class="item-label">负责人</label>
				...
				<el-row class="actions" :style='{"padding":"20px 20px 10px","margin":"20px 0 0","borderColor":"#355749","color":"#fff","textAlign":"left","flexDirection":"row","display":"block","borderRadius":"4px","flexWrap":"wrap","background":"#fff","borderWidth":"2px 0 0","width":"100%","fontSize":"inherit","borderStyle":"solid"}'>
					<el-button class="add" v-if="isAuth('shilinyeju','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('shilinyeju','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #e1e8ee","padding":"0px","margin":"0px auto","borderColor":"#355749","borderRadius":"4px","background":"rgba(255,255,255,1)","borderWidth":"2px 0 0","width":"calc(100% - 40px)","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shilinyeju','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('shilinyeju','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shilinyeju','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.linyejumingcheng!='' && this.searchForm.linyejumingcheng!=undefined){
            params['linyejumingcheng'] = '%' + this.searchForm.linyejumingcheng + '%'
          }
	...
	shilinyejustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'shilinyeju/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
